...
module.exports.server = function(serv, options) {
  ...
  serv.log.info('Starting numerous-alpaca server version 0.15.0 (' + version + ')');
...
module.exports.player = function(player, serv, settings) {
  player.login = function() {
    ...
    player._client.on('login_mcpe', function(packet) {
      player.spawn(packet);
    });
    ...
    player._client.on('end', function() {
      ...
      const index2 = findWithAttr(serv.list, 'client_uuid', player.uuid);
      if (index2 > -1) {
